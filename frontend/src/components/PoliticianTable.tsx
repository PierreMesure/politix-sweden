...
export default function PoliticianTable({ politicians, loading, activePlatform }: PoliticianTableProps) {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 md:shadow md:rounded-lg overflow-hidden md:border border-gray-200 dark:border-zinc-800 -mx-3 md:mx-0">
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-zinc-800">
          <thead className="bg-gray-50 dark:bg-zinc-950">
            <tr>
              <th className="pl-3 pr-1 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t('table.name')}</th>
                                      ...
                                      <td className="px-1 py-4 whitespace-nowrap text-sm">
                                        {p.social.bluesky ? (
                                          <div className="flex flex-col items-center lg:items-start">
                                            {/* Mobile: Icon (only shown if viewing 'all') */}
                                            {activePlatform === 'all' && (
                                              <a
                                            ...
                                            {/* Handle: Shown on desktop OR on mobile if platform is active */}
                                            <a
                                              href={`https://bsky.app/profile/${p.social.bluesky.handle}`}
                                              target="_blank"
                                              rel="noreferrer"
